...
export default function ListaProdutos({ history }) {
  ...
  useEffect(() => {
    function fetchData() {
      api
        .get('/providerproducts?brandproduct&provider', useAuth)
        .then(res => setProviderProducts(res.data));
      api.get('/products?category&brand&unity', useAuth).then(res => setProducts(res.data));
      api
        .get('/brandproducts?brand&product&stock', useAuth)
